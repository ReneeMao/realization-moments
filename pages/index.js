...
const pS4 = (card, story, s1, focal, cr) => {
  ...
  return `${SYS}\n\nSTAGE: REVISION CHECK-BACK\nEntry: "${card}"\nStory: "${story}"\nSummary: "${s1}"\nFocal: "${focal}"\nReflections:\n${ct}\n\nGenerate 3-5 short tentative statements. One sentence each ("It seems like..."), grounded in their words.\nJSON array of strings. ONLY JSON.`
}

const pS5 = (type, conf, story, focal) => {
  const inst = {
    reflective: 'REFLECTIVE NOTE: 4-6 sentences, second person ("You described..."), warm, provisional.',
    firstperson: 'FIRST-PERSON DRAFT: Short paragraph as if the person were speaking. Sound like them.',
    values: 'VALUES-AND-SHIFTS NOTE: 3-5 sentences. What matters, what\'s shifting, what stayed. Tentative.',
  }
...
function Journey({data,onEdit,onExport}){
  ...
  const outLabel={reflective:'Looking back',firstperson:'In my words',values:'What matters now'}[data.outputType]||'Your artifact'
  return(
...
export default function Home(){
  ...
  const sd=useCallback(()=>({timestamp:Date.now(),entryCard:selC?.label,userStory:story,stage1Response:s1,focalPointText:focal,cardResponses:cR,confirmedStatements:rvS.filter((_,i)=>rvM[i]==='fits'||rvM[i]==='notquite'),outputType:oT,outputText:oTx}),[selC,story,s1,focal,cR,rvS,rvM,oT,oTx])
